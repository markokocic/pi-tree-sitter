 ...
 */
import { describePrefixLeftover, findPrefixLeftover, simulateEditContent } from "../src/edit-guard.ts";
...
async function simulate(rawContent, edits) {
...
describe("edit simulation", () => {
  it("applies a byte-exact edit", async () => {
    const content = await simulate(FILE, [{ oldText: `    (t/is (map? all))`, newText: `    (t/is (map? all) true)` }]);
    assert(content !== null);
    assert(content.includes(`(t/is (map? all) true)`));
    assert.equal(findPrefixLeftover(FILE, [{ oldText: `    (t/is (map? all))`, newText: `    (t/is (map? all) true)` }]), undefined);
  });
  ...
  it("detects a prefix match that leaves closing delimiters (ticket repro)", async () => {
    ...
    const edits = [{ oldText: `    (t/is (contains? all "bash"))`, newText: `    (t/is (contains? all "bash"))  ; tuned` }];
  ...
  it("returns null when edits overlap", async () => {
    assert.equal(await simulate(FILE, [
  ...
  it("returns null when the replacement would not change the content", async () => {
    assert.equal(await simulate(FILE, [{ oldText: `(t/is (map? all))`, newText: `(t/is (map? all))` }]), null);
  });

  it("matches oldText with LF newlines against a CRLF file and restores endings", async () => {
    const crlf = FILE.replace(/\n/g, "\r\n");
    const content = await simulate(crlf, [{ oldText: `(t/is (map? all))`, newText: `(t/is (map? all) true)` }]);
    assert(content !== null);
    assert(content.includes(`(t/is (map? all) true)`));
  ...
  it("strips a UTF-8 BOM before matching and preserves it in the result", async () => {
    const content = await simulate("\uFEFF" + FILE, [{ oldText: "(ns guard-repro", newText: "(ns guard-repro2" }]);
    assert(content !== null);
  ...
  it("falls back to fuzzy matching for trailing whitespace in oldText", async () => {
    ...
    const content = await simulate(withWs, [{ oldText: `    (t/is (map? all))  `, newText: `    (t/is (map? all))` }]);
    assert(content !== null);
  ...
  it("reports the correct edit index in a multi-edit call", async () => {
    ...
    const edits = [
      ...
      { oldText: `    (t/is (contains? all "bash"))`, newText: `    (t/is (contains? all "bash"))  ; tuned` },
    ];
    assert((await simulate(FILE, edits)) !== null);
    const leftover = findPrefixLeftover(FILE, edits);
    assert(leftover);
  ...
  it("truncates long leftovers in the diagnosis message", () => {
    const longLine = FILE.replace("    (t/is (contains? all \"bash\"))))", "    (t/is (contains? all \"bash\"))" + ")".repeat(60));
    const edits = [{ oldText: `    (t/is (contains? all "bash"))`, newText: `    (t/is (contains? all "bash"))  ; tuned` }];
