...
import { createEditToolDefinition, type EditToolInput, type ExtensionContext } from "@earendil-works/pi-coding-agent";
 ...
 */
export function findPrefixLeftover(rawContent: string, edits: EditToolInput["edits"]): PrefixLeftover | undefined {
  ...
  for (let i = 0; i < normalized.length; i++) {
    const oldText = anyFuzzy ? normalizeForFuzzyMatch(normalized[i]) : normalized[i];
    ...
    if (/[)\]}]$/.test(oldText.trimEnd()) && /^[)\]}]/.test(restOfLine)) {
      ...
      return { editIndex: i, line, leftover: restOfLine };
    }
  }
  return undefined;
}
